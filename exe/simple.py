...
canvas = tk.Canvas(window, width=400, height=135, bg='#AEEEEE')  # 创建画布，用来放置图片
image_file = tk.PhotoImage(file='../src/5.gif')  # 本地图片转换
image = canvas.create_image(200, 0, anchor='n', image=image_file)
canvas.pack(side='top')
tk.Label(window, text='Welcome', font=('Arial', 16)).pack()

# 第5步，用户信息
tk.Label(window, text='User name:', font=('Arial', 14)).place(x=20, y=170)
tk.Label(window, text='  Password:', font=('Arial', 14)).place(x=20, y=210)
...
entry_usr_name.place(x=130, y=175)
# 用户密码
var_usr_pwd = tk.StringVar()
entry_usr_pwd = tk.Entry(window, textvariable=var_usr_pwd, font=('Arial', 14), show='*')
entry_usr_pwd.place(x=130, y=215)


var1 = tk.IntVar()
c1 = tk.Checkbutton(window, text='记住我',underline=1,variable=var1, onvalue=1, offvalue=0)    # 传值原理类似于radiobutton部件
c1.place(x=125, y=240)
...
btn_login.place(x=120, y=275)
btn_sign_up = tk.Button(window, text='Sign up', command=usr_sign_up)
btn_sign_up.place(x=200, y=275)
